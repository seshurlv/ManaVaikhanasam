	...
	.controller('tableController', function ($scope, $filter, ngTableParams) {
		...
		$scope.usersTable = new ngTableParams({
				...
				getData: function ($defer, params) {
				  ...
				  $scope.userdata = params.filter() ? $filter('filter')($scope.users, params.filter().userFullName) : $scope.users;
				  $scope.userdata = params.filter() ? $filter('filter')($scope.userdata, params.filter().PLACE) : $scope.userdata;
				  $scope.userdata = params.filter() ? $filter('filter')($scope.userdata, params.filter().AMOUNT) : $scope.userdata;
				  $scope.userdata = params.filter() ? $filter('filter')($scope.userdata, params.filter().DATE) : $scope.userdata;
				  $scope.userdata = params.filter() ? $filter('filter')($scope.userdata, params.filter().RECEIPT) : $scope.userdata;				  
				  params.total($scope.userdata.length);
				  $scope.userdata = $scope.userdata.slice((params.page() - 1) * params.count(), params.page() * params.count());				  				  
				  $defer.resolve($scope.userdata);
				}				
			});

		$scope.getAshramDonorsFullName = function(user) {
		   return user.userFullName = user.TITLE + ' ' + user.SURNAME + ' ' + user.NAME;
		};

		$scope.getItemDonorsFullName = function(item) {
		   return item.itemFullName = item.TITLE + ' ' + item.SURNAME + ' ' + item.NAME;
		};		
		...
		$scope.itemsTable = new ngTableParams({
				...
				getData: function ($defer, params) {
				  //params.settings().counts = [];                        
				  //$scope.data = params.sorting() ? $filter('orderBy')($scope.users, params.orderBy()) : $scope.users;
				  $scope.itemdata = params.filter() ? $filter('filter')($scope.items, params.filter().itemFullName) : $scope.items;
				  $scope.itemdata = params.filter() ? $filter('filter')($scope.itemdata, params.filter().PLACE) : $scope.itemdata;
				  $scope.itemdata = params.filter() ? $filter('filter')($scope.itemdata, params.filter().ITEM) : $scope.itemdata;
				  params.total($scope.itemdata.length);
